...
interface DentalDB extends DBSchema {
    ...
    assets: {
        ...
        value: {
            ...
            createdAt: string;
            /**
             * Binary payload of the asset. Absent (or `undefined`) when the
             * record is a pointer to an external file and only metadata is
             * kept on disk.
             */
            data?: Blob;
            /**
             * When `true`, this record intentionally has no `data` blob.
             * The viewer can detect this and surface a "re-link file"
             * message instead of trying to render the model.
             */
            isPointer?: boolean;
        };
    };
...
/** The name of the IndexedDB database used by the application (configurable via VITE_IDB_NAME for dev/prod separation). */
const DB_NAME = (import.meta.env?.VITE_IDB_NAME as string | undefined) || 'DentalRaktarDB';
...
export const dbService = {
    ...
    /**
     * Adds a pointer-only asset record to the database. The record is the
     * same shape as a regular asset (so the 3D viewer and the asset list
     * keep working) but the binary `data` blob is omitted and an
     * `isPointer: true` flag is set. The viewer can later use the metadata
     * (fileName, size) to ask the user to re-link the file from disk.
     *
     * @param asset - Metadata-only asset descriptor. `size` reflects the
     *                on-disk size in bytes.
     * @returns A promise that resolves when the pointer record is persisted.
     */
    async addPointerAsset(asset: { id: string; jobId: string; fileName: string; mimeType: string; size: number }): Promise<void> {
        const db = await initDB();
        await db.add('assets', { ...asset, createdAt: new Date().toISOString(), isPointer: true });
    },
