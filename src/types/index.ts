...
export interface JobAsset {
    ...
    /**
     * When `true`, the asset is a pointer to a 3D model on disk and the
     * `data` blob is intentionally absent from the IndexedDB record.
     * Pointer assets are kept for metadata-only workflows (e.g. to remember
     * the filename / size without paying the storage cost of the bytes).
     */
    isPointer?: boolean;
}
