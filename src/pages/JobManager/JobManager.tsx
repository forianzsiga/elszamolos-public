...
/**
 * Formats a byte count as a short, human-readable string. The 3D size gate
 * dialog uses this to show totals like "1.4 GB" or "780 MB" without
 * pulling in a dependency for one call site.
 *
 * - < 1 KiB     -> "{bytes} B"
 * - < 1 MiB     -> "{value} KB"
 * - < 1 GiB     -> "{value} MB"
 * - >= 1 GiB    -> "{value} GB"
 *
 * The value is always shown with one decimal place, except for the
 * sub-KiB case where bytes are exact integers.
 *
 * @param bytes - The number of bytes to format. Must be non-negative.
 * @returns A short label such as `"1.4 GB"`.
 */
...
const JobManagerPage = () => {
    ...
    /**
     * Pending 3D size-gate dialog state. When `totalBytes` is non-null the
     * dialog is open and a `resolve` callback is waiting for the user to
     * pick `'full'` (import the bytes) or `'pointer'` (metadata only).
     */
    const [gateDialog, setGateDialog] = useState<{
        totalBytes: number;
        resolve: ((mode: ThreeDImportMode) => void) | null;
    } | null>(null);

    /**
     * Resolves the active gate dialog, if any, with the user's choice.
     * Used as the `onClose` handler for the MUI Dialog so the user can
     * dismiss it (Esc / backdrop click) and still get a deterministic
     * "pointer" default instead of a hung Promise.
     */
    const resolveGateDialog = useCallback((mode: ThreeDImportMode) => {
        setGateDialog((current) => {
            if (current?.resolve) current.resolve(mode);
            return null;
        });
    }, []);

    /**
     * Adapter that turns the gate dialog state into the
     * `(totalBytes) => Promise<ThreeDImportMode>` shape expected by
     * {@link scanAndParseFolder}. Each call opens a new dialog and
     * resolves when the user clicks one of the two buttons (or
     * dismisses, which defaults to `'pointer'`).
     */
    const prompt3DImportFn = useCallback((totalBytes: number): Promise<ThreeDImportMode> => {
        return new Promise<ThreeDImportMode>((resolve) => {
            setGateDialog({ totalBytes, resolve });
        });
    }, []);
     ...
     *
     * If the total size of all 3D model files is at or above
     * {@link THREE_D_IMPORT_SIZE_LIMIT_BYTES} (1 GiB) the user is asked
     * — via {@link prompt3DImportFn} — whether to import the full model
     * data into IndexedDB or just pointers. Below the gate the function
     * imports the full data automatically.
     *
    ...
    const handleImport = async () => {
        ...
        try {
            const {
                ...
                types,
                importMode,
            } = await scanAndParseFolder(jobState.jobs, prompt3DImportFn);
            ...
            // Surface the chosen 3D import mode in the developer log so the
            // decision is auditable. Pointer mode is the safety default; the
            // user explicitly opted into full mode if we recorded it.
            if (importMode === 'pointer') {
                addLog(
                    'Imported 3D models as pointers (metadata only).',
                    'info',
                    'Re-link files from disk before viewing in the 3D viewer.'
                );
            }
        } catch (error) {
            console.error(error);
            // If a gate dialog is still open, resolve it as 'pointer' so the
            // scan promise (and the awaiter) does not hang when the
            // scanner threw.
            resolveGateDialog('pointer');
            setNotification({ msg: t('jobs.notifications.importError'), type: 'error' });
        } finally {
            setImporting(false);
        }
    ...
    return (
        <Box className="jobManagerContainer">
            ...
            <Dialog
                open={!!gateDialog}
                onClose={() => resolveGateDialog('pointer')}
                aria-labelledby="three-d-gate-title"
                aria-describedby="three-d-gate-description"
            >
                <DialogTitle id="three-d-gate-title">
                    {localT('threeDGateTitle')}
                </DialogTitle>
                <DialogContent>
                    <DialogContentText id="three-d-gate-description" className="threeDGateDescription">
                        {localT('threeDGateMessage', {
                            size: gateDialog ? formatBytes(gateDialog.totalBytes) : '0 B',
                        })}
                    </DialogContentText>
                    <DialogContentText variant="body2" className="threeDGateHint">
                        {localT('threeDGatePointerHint')}
                    </DialogContentText>
                </DialogContent>
                <DialogActions className="threeDGateActions">
                    <ResponsiveTooltip title={localT('threeDGateFullTooltip')}>
                        <Button
                            onClick={() => resolveGateDialog('full')}
                            color="primary"
                            variant="outlined"
                            fullWidth
                        >
                            {gateDialog
                                ? localT('threeDGateFull', { size: formatBytes(gateDialog.totalBytes) })
                                : ''}
                        </Button>
                    </ResponsiveTooltip>
                    <ResponsiveTooltip title={localT('threeDGatePointerTooltip')}>
                        <Button
                            onClick={() => resolveGateDialog('pointer')}
                            color="secondary"
                            variant="contained"
                            autoFocus
                            fullWidth
                        >
                            {localT('threeDGatePointer')}
                        </Button>
                    </ResponsiveTooltip>
                </DialogActions>
            </Dialog>
